          ...
          {activeTab === 'identificacion' && (
            <div className="row g-3">
              ...
              <div className="col-md-4">
                ...
                <ModernSelect className="form-select" value={tipoIdentificacion} onChange={e => { setTipoIdentificacion(e.target.value as 'CEDULA' | 'EXTRANJERO'); setCedula(''); }}>
              ...
              {(!edad || edad.años < 25) && (
                <div className="col-md-4"><label className="form-label fw-bold small">Año Escolar</label><ModernSelect className="form-select" value={anioEscolar} onChange={e => setAnioEscolar(e.target.value)}><option value="">Seleccione...</option><option value="Inicial 1">Inicial 1</option><option value="Inicial 2">Inicial 2</option><option value="1ro Básica">1ro Básica</option><option value="2do Básica">2do Básica</option><option value="3ro Básica">3ro Básica</option><option value="4to Básica">4to Básica</option><option value="5to Básica">5to Básica</option><option value="6to Básica">6to Básica</option><option value="7mo Básica">7mo Básica</option><option value="8vo Básica">8vo Básica</option><option value="9no Básica">9no Básica</option><option value="10mo Básica">10mo Básica</option><option value="1ro Bachillerato">1ro Bachillerato</option><option value="2do Bachillerato">2do Bachillerato</option><option value="3ro Bachillerato">3ro Bachillerato</option><option value="Universidad">Universidad</option><option value="No aplica">No aplica</option></ModernSelect></div>
              )}
              <div className="col-md-4"><label className="form-label fw-bold small">Sexo</label><ModernSelect className="form-select" value={sexo} onChange={e => setSexo(e.target.value)}><option value="">Seleccione...</option>{sexos.map(s => <option key={s.codigo} value={s.nombre}>{s.nombre}</option>)}</ModernSelect></div>
              <div className="col-md-4"><label className="form-label fw-bold small">Tipo de Sangre</label><ModernSelect className="form-select" value={tipoSangre} onChange={e => setTipoSangre(e.target.value)}><option value="">Seleccione...</option>{tiposSangreList.length > 0 ? tiposSangreList.map(t => <option key={t.codigo} value={t.nombre}>{t.nombre}</option>) : <><option value="A+">A+</option><option value="A-">A-</option><option value="B+">B+</option><option value="B-">B-</option><option value="AB+">AB+</option><option value="AB-">AB-</option><option value="O+">O+</option><option value="O-">O-</option></>}</ModernSelect></div>
              <div className="col-md-4"><label className="form-label fw-bold small">Grupo Étnico</label><ModernSelect className="form-select" value={grupoEtnico} onChange={e => setGrupoEtnico(e.target.value)}><option value="">Seleccione...</option>{etnias.length > 0 ? etnias.map(e => <option key={e.codigo} value={e.nombre}>{e.nombre}</option>) : <><option value="Mestizo">Mestizo</option><option value="Blanco">Blanco</option><option value="Indígena">Indígena</option><option value="Afroecuatoriano">Afroecuatoriano</option></>}</ModernSelect></div>
              <div className="col-12 mt-4"><h6 className="text-muted border-bottom pb-2">Ubicación Geográfica del Paciente</h6></div>
              <div className="col-md-4"><label className="form-label fw-bold small">Provincia</label><ModernSelect className="form-select" value={provincia} onChange={e => handleProvinciaChange(e.target.value)}><option value="">Seleccione...</option>{provincias.map(p => <option key={p.codigo} value={p.nombre}>{p.nombre}</option>)}</ModernSelect></div>
              <div className="col-md-4"><label className="form-label fw-bold small">Cantón</label><ModernSelect className="form-select" value={canton} onChange={e => handleCantonChange(e.target.value)} disabled={!provincia}><option value="">Seleccione...</option>{cantonesList.map(c => <option key={c.codigo} value={c.nombre}>{c.nombre}</option>)}</ModernSelect></div>
              <div className="col-md-4"><label className="form-label fw-bold small">Parroquia</label><ModernSelect className="form-select" value={parroquia} onChange={e => setParroquia(e.target.value)} disabled={!canton}><option value="">Seleccione...</option>{parroquiasList.map(p => <option key={p.codigo} value={p.nombre}>{p.nombre}</option>)}</ModernSelect></div>
            </div>
          )}
          {activeTab === 'filiacion' && (
            <div className="row g-3">
              ...
              <div className="col-md-4"><label className="form-label fw-bold small">Parentesco con el Paciente <span className="text-danger">*</span></label><ModernSelect className={`form-select ${errores.parentesco ? 'is-invalid' : ''}`} value={parentesco} onChange={e => setParentesco(e.target.value)}><option value="">Seleccione...</option>{parentescosList.map(p => <option key={p.codigo} value={p.nombre}>{p.nombre}</option>)}</ModernSelect></div>
              ...
              <div className="col-md-4"><label className="form-label fw-bold small">Nivel Educativo</label><ModernSelect className="form-select" value={nivelEducativoResponsable} onChange={e => setNivelEducativoResponsable(e.target.value)}><option value="">Seleccione...</option>{nivelesEducativosList.map(n => <option key={n.codigo} value={n.nombre}>{n.nombre}</option>)}</ModernSelect></div>
              <div className="col-12 mt-3"><h6 className="text-muted border-bottom pb-2">Ubicación del Responsable</h6></div>
              <div className="col-md-4"><label className="form-label fw-bold small">Provincia</label><ModernSelect className="form-select" value={provinciaRes} onChange={e => handleProvinciaResChange(e.target.value)}><option value="">Seleccione...</option>{provincias.map(p => <option key={p.codigo} value={p.nombre}>{p.nombre}</option>)}</ModernSelect></div>
              <div className="col-md-4"><label className="form-label fw-bold small">Cantón</label><ModernSelect className="form-select" value={cantonRes} onChange={e => handleCantonResChange(e.target.value)} disabled={!provinciaRes}><option value="">Seleccione...</option>{cantonesResList.map(c => <option key={c.codigo} value={c.nombre}>{c.nombre}</option>)}</ModernSelect></div>
              <div className="col-md-4"><label className="form-label fw-bold small">Parroquia</label><ModernSelect className="form-select" value={parroquiaRes} onChange={e => setParroquiaRes(e.target.value)} disabled={!cantonRes}><option value="">Seleccione...</option>{parroquiasResList.map(p => <option key={p.codigo} value={p.nombre}>{p.nombre}</option>)}</ModernSelect></div>
